...
`⟨d⟩`.- `(numer ⟨x⟩)` returns the numerator of the rational number 
`⟨x⟩`.- `(denom ⟨x⟩)` returns the denominator of the rational number 
`⟨x⟩`.
...
\frac{n_{1}}{d_{1}} & = & \frac{n_{2}}{d_{2}}\;\text{ }if\text{ }and\text{ }only\text{ }if\;n_{1}d_{2} = n_{2}d_{1}.
\end{array}$$
...
![Figure 2.1:Data-abstraction barriers in the rational-number package.](figures/Fig2.1d.std.svg)
...
parallel equivalent resistance $R_{p}$ of two resistors $R_{1}$ and $R_{2}$
using the formula
